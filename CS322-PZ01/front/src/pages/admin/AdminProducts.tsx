  ...
  return (
    ...
    >
      <input
        ...
        type="text"
        className="w-48 max-w-lg overflow-hidden text-wrap text-xl"
        ...
        {!!categoriesFiltered && categoriesFiltered.length > 0 && (
          <Popover className="block">
            ...
            <PopoverPanel
              transition
              anchor="bottom"
              className="top-9 z-20 flex flex-col items-center gap-2 rounded-xl border-2 bg-accent-50 p-5 text-sm/6 transition duration-200 ease-in-out [--anchor-gap:var(--spacing-5)] data-[closed]:-translate-y-1 data-[closed]:opacity-0"
            >
              {categoriesFiltered &&
      ...
      <textarea 
        ...
        className="w-full max-w-lg overflow-hidden text-wrap text-base text-neutral-600"
      ...
      <textarea 
        ...
        className="w-full max-w-lg overflow-hidden text-wrap text-sm text-neutral-600"
      ...
      <button
        className="rounded-md bg-accent-500 px-5 text-base font-bold text-white hover:brightness-125"
        type="submit"
        disabled={isPending}
      >
        Save Changes
      ...
      <button
        className="absolute right-0 top-0 rounded-md bg-red-500 px-5 py-2 text-base font-bold text-white hover:brightness-125"
...
const ProductEditForm = ({
  ...
  return (
    <ProductForm
      isPending={isPending}
      product={{ ...product, descriptionLong: productFull?.descriptionLong ?? "", ...productFull }}
      onSubmit={onSubmit}
      cancelEditing={cancelEditing}
    />
  );
...
const ProductInfo = ({
  ...
  return (
    <div className="flex h-full flex-1 flex-col gap-2">
      ...
      <p className="max-h-15 max-w-lg overflow-hidden text-wrap text-base text-neutral-600">
        {product.description}
      </p>

      <p className="max-h-15 max-w-lg overflow-hidden text-wrap text-sm text-neutral-600">
