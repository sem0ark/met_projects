...
const ProfileLink = () => {
  ...
  return (
    <NavLink
      ...
      className={({ isActive }) =>
        clsx(
          isActive ? "text-accent-400" : "hover:text-accent-700",
          "relative p-2 text-lg transition duration-100 ease-in-out",
        )
      }
    >
  ...
  return (
    <NavLink
      ...
      className={({ isActive }) =>
        clsx(
          isActive ? "text-accent-400" : "hover:text-accent-700",
          "relative rounded-lg border-2 border-slate-600 bg-slate-200 p-2 text-lg transition duration-100 ease-in-out hover:border-accent-600",
        )
      }
    >
      <CartIcon className="h-5" />
      {totalProducts > 0 && (
        <div className="absolute -right-1 -top-1 h-3 w-3 animate-ping rounded-full bg-accent-500"></div>
      )}
      {totalProducts > 0 && (
        <div className="absolute -right-1 -top-1 h-3 w-3 rounded-full bg-accent-500"></div>
      )}
    </NavLink>
  );
};
  ...
  return (
    <div
      className={clsx(
        ...
        "sticky z-10 flex h-16 w-full items-center border-b-2 border-accent-600 bg-white py-2 shadow-md duration-300 ease-in-out",
      )}
    >
      <div className="container mx-auto flex items-center gap-1 px-2">
        <Link
          to="/"
          className="flex flex-1 items-center text-2xl font-bold text-accent-700"
        >
          True C<BrandIcon />
          lors
        </Link>
...
export const MainLayout = () => (
  <div className="flex h-fit min-h-screen flex-col bg-accent-50/50">
    <Navigation />

    <main className="container relative mx-auto my-5 flex max-w-screen-lg flex-1 flex-col items-center justify-center">
      <Outlet />
    </main>

    <div className="z-10 h-fit w-full border-t-2 border-accent-600 bg-white py-2 text-accent-700 shadow-md">
      <Footer />
    </div>
  </div>
);
