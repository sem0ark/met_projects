    ...
    <tr className="odd:bg-neutral-100">
      <td className="flex w-full flex-nowrap items-center gap-2 px-2 sm:p-2 sm:px-6">
        ...
        <Button aria-label="Remove" onClick={() => removeProduct(productId)}>
          <CrossIcon className="h-8 text-accent-600 transition-transform duration-300 hover:scale-125" />
        </Button>
      </td>
  ...
  return (
    <>
      <Button
        ...
        className="rounded-md bg-black/20 px-4 py-2 text-sm font-medium text-black focus:outline-none data-[hover]:bg-black/30 data-[focus]:outline-1 data-[focus]:outline-white"
      >
        Checkout
      </Button>
      ...
      >
        <div className="fixed inset-0 z-10 w-screen overflow-y-auto backdrop-blur-sm">
          <div className="flex min-h-full items-center justify-center p-4">
            <DialogPanel className="data-[closed]:transform-[scale(95%)] w-full max-w-md rounded-xl border-2 border-accent-500 bg-accent-100 p-6 duration-300 ease-out data-[closed]:opacity-0">
              <DialogTitle
                as="h3"
  ...
  if (cartItems.length === 0)
    return (
      <div className="flex h-[80vh] flex-col items-center justify-center gap-5">
        ...
        <Link
          to="/products"
          className="animate-pulse rounded-full border-2 border-accent-700 bg-accent-50 px-5 py-2 hover:animate-none"
        >
          See Products
        </Link>
      </div>
