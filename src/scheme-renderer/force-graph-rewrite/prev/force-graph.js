...
export default Kapsule({
  ...
  init: function (domNode, state) {
    ...
    const getObjUnderPointer = () => {
      ...
      if (px) {
        obj = state.colorTracker.lookup(px.data);
      }
      return obj;
    };
    ...
    state.zoom
      ...
      .on("zoom", (ev) => {
        ...
        state.isPointerDragging = true;
        state?.onZoom?.({ ...t, ...this.centerAt() }); // report x,y coordinates relative to canvas center
        state.needsRedraw = true;
      })
      .on("end", (ev) => {
        state.isPointerDragging = false;
        state?.onZoomEnd?.({ ...ev.transform, ...this.centerAt() });
      });
    ...
    state.forceGraph
      ...
      .onFinishUpdate(() => {
        ...
        if (
          d3ZoomTransform(state.canvas).k === state.lastSetZoom &&
        ...
        (ev) => {
          ...
          if (
            ...
            (ev.pointerType === "mouse" ||
              ...
              [ev.movementX, ev.movementY].some((m) => Math.abs(m) > 1)) // relax drag trigger sensitivity on non-mouse (touch/pen) events
          ) {
            state.isPointerDragging = true;
          }
      ...
      (ev) => {
        ...
        requestAnimationFrame(() => {
          ...
          if (ev.button === 0) {
            ...
            if (state.hoverObj) {
              state[`on${state.hoverObj.type}Click`]?.(
                state.hoverObj.d,
                ...cbEvents,
              );
            } else {
          ...
          if (ev.button === 2) {
            // mouse right-click
            if (state.hoverObj) {
              state[`on${state.hoverObj.type}RightClick`]?.(
                state.hoverObj.d,
                ...cbEvents,
              );
            } else {
    ...
    const refreshShadowCanvas = throttle(() => {
      ...
      state.shadowGraph.linkWidth(
        (l) => accessorFn(state.linkWidth)(l) + state.linkHoverPrecision,
      );
      ...
      if (state.enablePointerInteraction) {
        ...
        if (obj !== state.hoverObj) {
          ...
          state.canvas.classList[
            ...
            accessorFn(state.showPointerCursor)(obj?.d)
              ? "add"
              : "remove"
          ]("clickable");
