...
const ProductCard = ({ product }: { product: Product }) => {
  return (
    <div className="relative flex w-40 max-w-40 flex-col gap-2 rounded-md shadow-md">
      ...
      <p className="overflow-hidden text-center text-base text-ellipsis">
        {product.name}
      </p>
...
const CategoryHeading = ({ id }: { id: number }) => {
  const { data, isSuccess } = useQuery_FetchCategory(id);
  return (
    isSuccess && (
      <div className="flex h-10 cursor-pointer flex-row items-center rounded-full px-2 text-2xl text-nowrap transition-colors duration-300 ease-in-out">
...
const Filters = ({
  ...
  const className =
    "flex h-5 w-fit cursor-pointer flex-row items-center text-nowrap rounded-full border-2 border-orange-600 bg-orange-600 px-2 text-xs text-white transition-colors duration-300 ease-in-out hover:bg-orange-300";
...
export function Products() {
  ...
  const filteredByCategory = categoryIdParam
    ? data?.filter((it) =>
        it.categoryIds.includes(Number.parseInt(categoryIdParam)),
      )
    : data;
    ...
    <div className="flex flex-col items-center gap-5">
      ...
      {!!categoryIdParam && (
        <div className="mb-10 flex w-full items-center justify-center">
          <CategoryHeading id={Number.parseInt(categoryIdParam)} />
        </div>
      )}
